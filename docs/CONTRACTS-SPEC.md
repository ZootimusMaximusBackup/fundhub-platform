...
Three triggers, in `db/migrations/124_contracts.sql`:
...
**Database**
* `db/migrations/124_contracts.sql` — both tables, indexes, three triggers
...
| Storage | **The existing document store (src/documents/store.mjs), same as uploads** | §17 |
...
## 17. Storage: the same document store every other upload in this codebase uses

The owner said the files get saved in the blob. `src/documents/store.mjs` is the
one storage abstraction in this repository — it already carried a Vercel Blob
provider before this feature, and gained a Netlify Blobs provider in the same
merge window (the client-file-uploads build). Contracts store the uploaded PDF
and the finished signed PDF through this same module, via `storeFromEnv()` —
there is no second storage layer here or anywhere else in the codebase.

`providerFromEnv()` selects on `DOCUMENT_STORE_PROVIDER` when it is set, and
defaults to the in-memory provider when it is not — production must set the
variable explicitly; `storeFromEnv()` warns once if it has not. (An earlier
draft of this section proposed a Postgres fallback provider, auto-selected from
`BLOB_READ_WRITE_TOKEN`/`DATABASE_URL`, so contracts would work before any
storage vendor was configured. That was a genuine conflict with the
already-merged uploads branch's simpler "explicit env var, memory otherwise"
design, decided in the merge review in favor of the single existing store
module — see `docs/MERGE-LOG.md` section 3. The Postgres provider and its
`document_blobs` table were removed from `125_contract_esign.sql` accordingly;
nothing in `src/contracts/` ever wrote to `document_blobs` directly, so no
call-site changes were needed beyond this doc.)

> **Operator action, when convenient:** set `DOCUMENT_STORE_PROVIDER` to
> `vercel-blob` or `netlify-blobs` (with the matching credential) and every
> document — uploads and contracts alike — moves to that store. Nothing above
> the provider interface knows or cares which one it is talking to.
...
**Database** — `db/migrations/125_contract_esign.sql`: PDF columns on
...
`db/migrations/126_outbound_switch.sql` replaces it with

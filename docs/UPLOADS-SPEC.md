...
than force-fit one or bypass the registry, `db/migrations/118_client_uploads.sql`
...
| File | What it does |
|---|---|
| `db/migrations/118_client_uploads.sql` | Adds `kind = 'client_upload'` to the `documents` CHECK constraint. Finds and drops the old constraint by its definition (not a guessed name), so it can't silently no-op. |
...
- **`118_client_uploads.sql` needs to be applied to the real database.**

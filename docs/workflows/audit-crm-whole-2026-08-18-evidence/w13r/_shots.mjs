/**
 * W13R screenshots only. Does not Save, promote, demote, or create.
 */
...
const ag04 = page.locator('.aitem[data-a="AG-04"]');
if (await ag04.count()) {
  await ag04.click();
  await page.waitForTimeout(400);
  await page.screenshot({ path: path.join(OUT, "02-ag-04.png"), fullPage: false });
}

const ag09 = page.locator('.aitem[data-a="AG-09"]');
if (await ag09.count()) {
  await ag09.click();
  await page.waitForTimeout(400);
  await page.screenshot({ path: path.join(OUT, "03-ag-09.png"), fullPage: false });
}

const facts = await page.evaluate(() => {
  const items = [...document.querySelectorAll(".aitem")].map((el) => ({
    code: el.getAttribute("data-a"),
...
await browser.close();
fs.writeFileSync(path.join(OUT, "screen.json"), JSON.stringify({ at: new Date().toISOString(), facts, saved: false, promoted: false }, null, 2));
console.log(JSON.stringify({ url: facts.url, kAll: facts.kAll, kLive: facts.kLive, n: facts.items.length }));

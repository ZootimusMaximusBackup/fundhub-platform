...
| 6 | Payout generator (GAP 3) | opus | **done** |
...
- **Task 6 — ANSWERED, OWNER-SET 2026-09-21.** Chris was asked for the three numbers and
  said to pick them. Picked, all three overridable per call:
  **monthly** over the previous whole calendar month; **$50 minimum**, with anything under
  it writing nothing at all so the commission rolls forward by itself; **an unsigned
  license or missing tax form holds the run** rather than skipping it, so the money stays
  counted and visible while it cannot leave.

### Task 6 manifest — 2026-09-21

**COMPLIANCE REVIEW REQUIRED: payment rails.**

| File | What |
|---|---|
| `src/affiliates/payouts.mjs` | NEW — `buildPayoutRun()`, `buildPayoutRunsForAllOrgs()`, `previousMonth()`, `payoutKey()`, `PAYOUT_DEFAULTS`. |
| `src/workflows/affiliate-payout-run.mjs` | NEW — the monthly cron, `0 3 1 * *`. |
| `src/workflows/index.mjs` | Registers it. |
| `src/workflows/index.test.mjs` | Pinned count 72 → 73, with the reason. |
| `src/journeys/runner/index.test.mjs` | `REGISTERED` 72 → 73, with the reason. |
| `src/affiliates/payouts.pg.test.mjs` | NEW — 17 tests against a real database. |
| `docs/diagrams/*` | Regenerated by `npm run diagrams`. |

**It moves no money and cannot.** It writes `pending` and `held` rows. `processing` and
`paid` are a human action against a payment rail this repository does not contain, and
`affiliate_payouts_guard()` (`033_affiliates.sql`) refuses that move for any affiliate
with no signed partner license. There is no outbound `fetch` in either new file.

**Double-paying is prevented by the schema, not by the new code.**
`affiliate_payout_lines_commission_once` is a UNIQUE index on `referral_id`. Two runs
racing the same converted referral cannot both line it. The idempotency key is derived
from the affiliate and the period and nothing else, so a re-run collides on
`affiliate_payouts_idem_uniq` instead of building a second run. Both paths have a test.

**Measured** — scratch Postgres 16, all migrations applied to an empty database:

| Check | Result |
|---|---|
| `src/affiliates/payouts.pg.test.mjs` | 17 tests, 17 pass, 0 fail, 0 skipped |
| `npm run lint` | clean, 1967 files |
| `npx tsc --noEmit` | clean |
| workflow registry + journeys + routes | 73 pass, 0 fail |
| `npm run diagrams:check` | up to date, 11 files |
| Full suite vs baseline | **no new failure by name**, and two baseline failures now fixed (journeys no longer stale, diagrams back in sync) — 10 named failures where baseline had 12 |

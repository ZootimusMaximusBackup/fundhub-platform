...
_FONTS_DIR = _Path(__file__).resolve().parent / "fonts"


def font_faces():
    """Bundle Inter + JetBrains Mono next to this template so prints match gold everywhere."""
    faces = []
    for family, weight, name in (
        ...
        if f.exists():
            faces.append(
                ...
                f'font-style: normal; src: url("{f.as_uri()}"); }}'
            )
    return "\n".join(faces)

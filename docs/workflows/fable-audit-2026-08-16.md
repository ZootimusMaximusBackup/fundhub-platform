...
**client · S4 landing — signed-in client sees an empty portal: "Open this from a client file" (FIXED-UNCLICKED)**
...
**Prove:** `npm run lint` clean. `node --test src/http/demo-logins.test.mjs` 45/45. `npm run test:e2e:live` 26/26 = **100/100**. Board S4 row → FIXED-UNCLICKED. Evidence: `docs/workflows/e2e-verify-run5-evidence/client/fixed/`. Waiting on Chris's one manual click after this ships.
...
| 6 | affiliate Company Brain Ask 401 (staff-only auth before role) | affiliate / affiliate@fundhub.ai | `api/read/company-brain-affiliate.mjs`, `src/http/company-brain.test.mjs` | FIXED-UNCLICKED |

Agents wrote evidence to `docs/workflows/e2e-verify-run5-evidence/<journey>/fixed/`. Parent applied board rows. All six **FIXED-UNCLICKED**.

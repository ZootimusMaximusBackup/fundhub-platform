...
**Owner law:** Pack PDFs must look **exactly** like the gold files in `docs/workflows/gold-deliverables-v5/`. Letters: no Fundhub branding. Charts: no model in the draw path. QR on report close page → `https://apply.fundhub.ai`.

**Bar:** empty letter body → do not ship. JSON dumped into a report → do not ship. `[ QR CODE ]` placeholder → do not ship.
...
| W8 Letter typeface Inter + JetBrains Mono | Claude rule (design) | **parked** — do not run on Grok |
...
- W8: font loader + `vendor/underwriteiq-full/api/lite/letter-generator.js` typeface only (do not change bodies)

#!/usr/bin/env python3
"""Burn numbered red boxes + yellow arrows + a caption legend onto the W4B proof shots.

Lifted unchanged in substance from docs/workflows/company-sim-2026-08-24-evidence/,
which is the tooling CLAUDE.md section 8 names. The one difference: the shot
directory is an argument, because this proof has a `before` set and an `after`
set and they must not overwrite each other.

    python3 _apply-marks.py shots-before
    python3 _apply-marks.py shots-after

Boxes come from e2e/w4b-portal-walk.spec.mjs, which reads each element's real
position in the browser, so a red box can never point at empty space."""
...
PHASE = sys.argv[1] if len(sys.argv) > 1 else "shots-after"
SHOTS = HERE / PHASE
RAW = SHOTS / "_raw"
MANIFEST = SHOTS / "shot-marks.json"
RED = (255, 40, 40, 255)

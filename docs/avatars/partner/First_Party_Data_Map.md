...
### Legacy GHL motivation columns — count before trusting
...
-- legacy free-text fields (GHL port only; check population):
...
7. **Check row counts before building on legacy columns.** The GHL-era fields (`cf_svy_your_why`, `cf_svy_what_matters_most`, `pre_call_question`, `funding_planned_use`, `repair_why`) have no current writer; population is unknown.

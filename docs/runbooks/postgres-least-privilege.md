...
```
cd ~/fundhub-platform
git fetch origin claude/postgres-superuser-migration-lzm6xt
git checkout claude/postgres-superuser-migration-lzm6xt
git pull origin claude/postgres-superuser-migration-lzm6xt
npm install
```

**You should see:** `Switched to branch 'claude/postgres-superuser-migration-lzm6xt'`, then npm
ending with `added ... packages` or `up to date`.

**If it says** `error: pathspec ... did not match` — the code has not arrived yet. Stop and tell Claude.
...
then **`✔ applied migrations/090_app_role.sql`**,

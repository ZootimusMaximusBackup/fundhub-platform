...
- `api/public/optimize.mjs` — GET returns `smartCredit: null` unless both a client key and a PID exist. POST ignores any client product title and always mints **Consulting Services Assessment** via `createCheckoutSession`. Never POST `/public-api/products/create`.

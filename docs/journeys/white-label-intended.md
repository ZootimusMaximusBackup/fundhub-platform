...
> extracted route data as [`white-label-actual.md`](./white-label-actual.md) —
...
What a principal of kind `partner` should be able to do.
...
```mermaid
flowchart TD
    START([white-label arrives]) --> AUTH{Signed in?}
    AUTH -->|No| OUT[Refused]
    ...
    WHO -->|Yes| CANT[Should stay blocked from — 66 routes]
...
Run `npm run journeys` and read [`white-label-actual.md`](./white-label-actual.md). There is no automated

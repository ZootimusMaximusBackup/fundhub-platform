...
- **No max content width. Screens fill the window.** *(Owner-set 2026-08-27. The cap is gone: `--fh-maxw` is `none`. History — 1280px, raised to 1800px on 2026-08-17, removed entirely on 2026-08-27. The reason it went is the reason it was raised, only louder: the owner zooms out on a 2560px monitor to get more CRM on screen, and any px cap defeats that exactly. Zooming out widens the viewport, the content stops growing at the cap, and every pixel gained becomes equal dead margin on both sides — reported as "I can't zoom out and there's side padding on the sides." The old bullet claimed full-bleed dashboards read as unfinished; the owner has looked at both and decided otherwise. The token is `--fh-maxw` in `public/app/fundhub-brand.css` and it is still the single place to change this — `.fh-maxw` there and the `!important` rule in `crm-sidebar.css` both resolve through it. Do not reintroduce a px cap.)*

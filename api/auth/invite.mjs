...
//
// { name, role } → { ok, email, token, expiresAt, invitePath, staff }
//
// The app makes name@fundhub.ai. That is a login, not a mailbox.
// The token comes back so Chris can copy the set-password link.
// Nothing here sends mail (CLAUDE.md §12).
...
export default async function handler(req, res) {
  ...
  return res.status(200).json({
    ...
    invitePath: "/reset-password.html?token=" + encodeURIComponent(result.token),
    staff: result.staff
  });
}

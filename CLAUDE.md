...
* **The suite is not as green as it looks.** With `DATABASE_URL` unset, ~193 `.pg.test.mjs` tests skip and the suite reports 0 failures. Against a real Postgres there are **45 pre-existing failures**. Verify with `DATABASE_URL` set, and diff against the baseline commit before concluding you broke something. Measured 2026-07-31 on a local Postgres 16.13, virgin database, all migrations applied, on `main` at `e67e2db`. The count did **not** change on a second run against the same database, so treat the earlier note about order-dependent `inquiries` suites settling from 29 to 24 as unconfirmed — it did not reproduce. That older figure may have been measured against Supabase rather than local Postgres; if you get a different number, record where you ran it, because the environment appears to move the count.

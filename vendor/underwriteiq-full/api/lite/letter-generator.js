...
async function embedGoldFont(pdfDoc, fontsDir, fileName, fallbackStandard) {
  ...
  if (fs.existsSync(filePath)) {
    return pdfDoc.embedFont(fs.readFileSync(filePath), { subset: true });
  }
  return pdfDoc.embedFont(fallbackStandard);
}
...
async function renderPlainLetter(spec) {
  ...
  const fontsDir = goldFontsDir();
  const font = await embedGoldFont(pdfDoc, fontsDir, "Inter-Regular.ttf", StandardFonts.Helvetica);
  const bold = await embedGoldFont(pdfDoc, fontsDir, "Inter-Bold.ttf", StandardFonts.HelveticaBold);
  const mono = await embedGoldFont(pdfDoc, fontsDir, "JetBrainsMono-Regular.ttf", StandardFonts.Courier);
  const monoBold = await embedGoldFont(pdfDoc, fontsDir, "JetBrainsMono-Bold.ttf", StandardFonts.CourierBold);
  try {

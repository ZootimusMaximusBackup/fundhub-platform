...
test("happy path: funding path sends the funding letter pack delivery", async () => {
  const db = pgFake({
    clients: [{ id: "cl-1", org_id: "org-1", email: "a@b.com", outcome_tier: "FULL_STACK_APPROVAL", custom_fields: {} }],
...
test("branch: repair path sends the repair letter pack delivery", async () => {
  const db = pgFake({
    clients: [{ id: "cl-1", org_id: "org-1", email: "a@b.com", outcome_tier: "REPAIR", custom_fields: {} }],

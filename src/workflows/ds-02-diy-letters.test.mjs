...
test("HARD RULE — DIY pay never calls PostGrid / mailBureauLetter", async () => {
  // The metro2 engine STORES dispute letter PDFs to the documents registry
  // for a human to bureau-mail later (see src/metro2/diy/deliver.mjs) — it
  // never mails a bureau itself, and DS-02 must not import or call anything
  // that would (that would bypass the required human review step).
...
test("duplicate delivery: replaying the same event does not double-send, double-task, or double-tag", async () => {
  ...
  let deliverCount = 0;
  const countingDeliver = async () => { deliverCount++; return { delivered: true, files: DIY_FILES }; };
  await handle({ event, db, step: fakeStep(), deliverPack: countingDeliver });
  await handle({ event, db, step: fakeStep(), deliverPack: countingDeliver });

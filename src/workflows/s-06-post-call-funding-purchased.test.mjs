...
test("happy path: funding-path deposit.paid tags + sets lifecycle, creates intake task", async () => {
  const db = pgFake({ clients: [{ id: "cl-1", org_id: "org-1", email: "a@b.com", outcome_tier: "FULL_STACK_APPROVAL", tags: ["client:repair"], custom_fields: {} }] });
...
test("duplicate delivery: replaying does not double-create the task", async () => {
  const db = pgFake({ clients: [{ id: "cl-1", org_id: "org-1", email: "a@b.com", outcome_tier: "FULL_STACK_APPROVAL", custom_fields: {} }] });

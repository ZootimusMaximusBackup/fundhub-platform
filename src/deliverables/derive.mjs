...
export function fileFactSentences(client) {
  ...
  // Main's wording, kept from the roadmap honest-prose work that shipped
  // 2026-09-06 and is pinned by roadmap-honest-prose.test.mjs: a plain English
  // list, not a colon and a comma-separated run.
  else if (clean.length > 1) {
    const names = clean.length === 2
      ? clean.join(" and ")
      : `${clean.slice(0, -1).join(", ")} and ${clean[clean.length - 1]}`;
    out.push(`Your ${names} files are clean.`);
  }
  return out;
}

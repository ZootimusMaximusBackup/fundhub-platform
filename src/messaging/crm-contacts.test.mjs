...
import { config, findOrCreateCrmContact, ensureCrmContactId } from "./crm-contacts.mjs";
...
/** A minimal db fake for ensureCrmContactId — records every UPDATE. */
function dbFake() {
  const updates = [];
  return {
    updates,
...
test("config: base URL and version default, and both env spellings override", () => {
  ...
  const viaCrm = config({ GHL_API_KEY: "k", GHL_BASE_URL: "https://a.example/", GHL_VERSION: "v2" });
  assert.equal(viaCrm.baseUrl, "https://a.example"); // trailing slash stripped
  assert.equal(viaCrm.version, "v2");
...
test("findOrCreate: phone-only contact (no email) still resolves", async () => {
  const fetchImpl = fakeFetch({ status: 200, body: { id: "crm-phone-1" } });
  const r = await findOrCreateCrmContact({ phone: "+15551234567" }, { env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" }, fetchImpl });
  assert.equal(r.ok, true);
  assert.equal(r.contactId, "crm-phone-1");
});
...
test("findOrCreate: a non-2xx, non-404/405 upsert response is reported, not guessed at", async () => {
  ...
  const r = await findOrCreateCrmContact({ email: "a@b.com" }, { env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" }, fetchImpl });
  assert.equal(r.ok, false);
...
test("findOrCreate: upsert 404 falls back to search — finds an existing contact by email", async () => {
  const fetchImpl = fakeFetch([
    ...
    { status: 200, body: { contacts: [{ id: "crm-found" }] } } // GET /contacts/?email=...
  ]);
  const r = await findOrCreateCrmContact({ email: "existing@b.com" }, { env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" }, fetchImpl });
  assert.equal(r.ok, true);
  assert.equal(r.contactId, "crm-found");
...
test("findOrCreate: upsert 405 falls back to search-then-create when search finds nothing", async () => {
  const fetchImpl = fakeFetch([
    ...
    { status: 200, body: { contact: { id: "crm-created" } } } // create
  ]);
  const r = await findOrCreateCrmContact({ email: "new@b.com", lastName: "Doe" }, { env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" }, fetchImpl });
  assert.equal(r.ok, true);
  assert.equal(r.contactId, "crm-created");
  assert.equal(r.created, true);
...
test("findOrCreate: GHL_LOCATION_ID is sent when the env carries one", async () => {
  const fetchImpl = fakeFetch({ status: 200, body: { id: "crm-loc" } });
...
test("ensureCrmContactId: writes clients.ghl_contact_id by default", async () => {
  const db = dbFake();
  const fetchImpl = fakeFetch({ status: 200, body: { id: "crm-write-1" } });
  const r = await ensureCrmContactId(
    db,
    { id: "cl-1", email: "a@b.com", phone: null, first_name: "A", last_name: "B" },
    { fetchImpl, env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" } }
  );
  assert.equal(r.ok, true);
  assert.equal(r.contactId, "crm-write-1");
  assert.equal(r.updated, true);
  assert.deepEqual(db.updates, [{ contactId: "crm-write-1", clientId: "cl-1" }]);
});

test("ensureCrmContactId: dryRun finds/creates but never writes — this is the backfill script's default", async () => {
  const db = dbFake();
  const fetchImpl = fakeFetch({ status: 200, body: { id: "crm-dry-1" } });
  const r = await ensureCrmContactId(
    db,
  ...
  assert.equal(r.ok, true);
  assert.equal(r.contactId, "crm-dry-1");
  assert.equal(r.updated, false);
  assert.deepEqual(db.updates, [], "dryRun must not touch the database");
});

test("ensureCrmContactId: no email/phone on the row — no_identifier, no CRM call", async () => {
  const db = dbFake();
  const fetchImpl = fakeFetch({ status: 200, body: { id: "should-not-be-called" } });
  const r = await ensureCrmContactId(db, { id: "cl-3" }, { fetchImpl, env: { ADAPTERS_DRY_RUN: "0", GHL_API_KEY: "k" } });
  assert.equal(r.ok, false);
  assert.equal(r.reason, "no_identifier");
  assert.equal(fetchImpl.calls.length, 0);
});

test("ensureCrmContactId: never throws, even if the db write itself fails", async () => {
  const fetchImpl = fakeFetch({ status: 200, body: { id: "crm-x" } });
...
test("ensureCrmContactId: propagates not_configured without ever calling the db", async () => {
  const db = dbFake();
  const r = await ensureCrmContactId(db, { id: "cl-5", email: "a@b.com" }, { env: {} });
  assert.equal(r.ok, false);
  assert.equal(r.reason, "not_configured");

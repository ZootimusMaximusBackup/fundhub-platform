...
describe("the provider registry", () => {
  ...
  test("an unknown provider resolves to null — a hold, never a default", () => {
    assert.strictEqual(resolve("twilio"), null);
  ...
  test("the two channels in the routing seed are each carried by exactly one provider", () => {
    for (const channel of ["email", "sms"]) {
      const carriers = Object.values(PROVIDERS).filter((p) => p.CHANNELS.has(channel));
      assert.strictEqual(carriers.length, 1, `${channel} should have exactly one provider`);
    }
  });
...
describe("provider contract, structurally", () => {
  const files = ["mailgun.mjs", "ghl-relay.mjs", "http.mjs", "index.mjs"];
  ...
  test("no credential is hardcoded — they come from the environment only", () => {
    for (const f of ["mailgun.mjs", "ghl-relay.mjs"]) {
      const code = codeOf(f);

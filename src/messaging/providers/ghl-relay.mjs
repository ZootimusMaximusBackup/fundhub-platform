// W3 — the SMS provider. The CRM LeadConnector v2, conversations endpoint.
//
// WHY the CRM AND NOT A CARRIER DIRECTLY. The cutover moves email off the CRM first and
...
// ones this repository already uses successfully against the CRM in
...
// { type: "SMS", contactId, message }) is the CRM's documented v2 send call.
//
// ASSUMPTION WORTH KNOWING, recorded rather than hidden: there is no CRM relay
...
// than the CRM directly, the only thing that changes is GHL_RELAY_BASE_URL, which
...
// differs from the email provider and the reason ADDRESS_FIELD exists. The CRM's
...
//   GHL_RELAY_BASE_URL   optional; defaults to the CRM's LeadConnector host
...
/** The CRM addresses by contact, so the dispatcher resolves clients.ghl_contact_id. */
export const ADDRESS_FIELD = "ghl_contact_id";

/** False: owner 2026-08-14 — the CRM account canceled. SMS routes to Twilio.
...
// The same version scripts/gen-custom-field-migration.mjs pins. The CRM requires the
...
/* send — the CRM is dead (owner 2026-08-14). Log and no-op so nothing crashes.
...
export async function send(message = {}, _options = {}) {
  try {
    ...
    console.info(`[ghl_relay] no-op — the CRM removed; message=${id} not sent`);

...
/* Deployment signals. NETLIFY, CONTEXT and BUILD_ID are set by Netlify during
   ...
   what "in any environment" requires). CI and GITHUB_ACTIONS cover a CI runner
   if one is ever added; this repo has no .github/ today, so the Netlify build
   is currently the only gate that exists. */
const DEPLOY_SIGNAL =
  ["NETLIFY", "CONTEXT", "BUILD_ID", "GITHUB_ACTIONS", "CI"].find((k) => process.env[k]) ??
...
test("the app's database role holds no superuser-level privilege", async (t) => {
  ...
  assert.deepStrictEqual(
    ...
    `\n\n` +
    `The app must connect as fundhub_app, created by db/migrations/090_app_role.sql.\n` +
    (DEPLOYED

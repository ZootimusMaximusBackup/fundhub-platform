...
const SALES_MANAGER_MIGRATION = path.join(ROOT, "db/migrations/111_sales_manager_role.sql");
...
describe("the roster is the same in all four places", () => {
  ...
  test("ten logins, one per role", () => {
    ...
    // (db/migrations/111_sales_manager_role.sql), plus the three principals.
  ...
  test("the migration seeds exactly this roster", () => {
    /* Two files now, not one. 094_demo_logins.sql was already applied when the
       ...
       silent no-op. So 111_sales_manager_role.sql carries that one login, and

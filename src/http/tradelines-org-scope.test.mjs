  ...
  return {
    ...
    async query(sql, params) {
      ...
      if (sql.includes("FROM tradelines")) {
        // params: [clientId, orgId, includeClosed]
...
describe("listTradelines refuses an unscoped read", () => {
  ...
  test("with both, the org reaches the query as a real parameter", async () => {
    ...
    assert.match(seen[0].sql, /org_id\s*=\s*\$2/, "the org must be in the WHERE clause, not just accepted");
    assert.equal(seen[0].params[1], ORG);
  });
});

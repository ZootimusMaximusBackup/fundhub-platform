...
function buildSuggestions(uw, user = {}) {
  ...
  if (!hasLLC && uw.fundable) {
    out.push("You’re approved, but you don’t have an LLC. Forming one now lets you unlock business funding immediately.");
  } else if (!hasLLC && !uw.fundable) {
    out.push("You don’t have an LLC yet. Form an LLC now so it can season while your credit is being repaired.");
  } else if (hasLLC && llcAge < 6) {
    out.push("Your LLC is under 6 months old. Approvals and limits improve significantly after it seasons past 6 months.");
  } else if (hasLLC && llcAge >= 6 && llcAge < 24) {
    out.push("Your LLC is seasoning well. Once it matures past 12–24 months, business approvals increase even more.");
  } else if (hasLLC && llcAge >= 24 && uw.fundable) {
    out.push("Your LLC is fully seasoned. Combined with a strong personal profile, you are positioned for top-tier business limits.");
  } else if (hasLLC && llcAge >= 24 && !uw.fundable) {
    out.push("Your LLC is seasoned. Once personal cleanup finishes, you will unlock the highest tiers of business approvals.");
  }
  ...
  if (out.length === 0) {
    if (uw.fundable) {
      out.push("Your profile is strong. The next step is proper sequencing and lender selection to maximize total approvals.");
    } else {
      out.push("You're close to approval. A few targeted improvements will push you into approval range.");
    }
  }

...
export function summary(run, diffed) {
  ...
  /* The live-mode fence is a pre-flight now, not a per-message status — see
     src/messaging/live-fence.mjs. A refused path dispatched nothing at all, so
     the count is of paths held back, not of individual messages. The compliance
     gate is separate and still per-message. */
  const fenceRefusals = run.paths.filter((p) => p.dispatchRefused);
  const blocked = run.paths.flatMap((p) => (p.dispatched || []).filter((d) => d.outcome === "blocked"));
  ...
  return {
    ...
    messages: {
      ...
      pathsRefusedByFence: fenceRefusals.length,
      fenceRefusals: fenceRefusals.map((p) => ({
        pathId: p.pathId,
        reason: p.dispatchRefused.reason,
        routes: p.dispatchRefused.routes
      })),
      blockedByGate: blocked.length
    },
    findings: {
...
export function text(run, diffed) {
  ...
  out.push(
    ...
    `  have gone out:      ${s.messages.wouldHaveGoneOut}` +
      (s.messages.pathsRefusedByFence
        ? `  (${s.messages.pathsRefusedByFence} path${s.messages.pathsRefusedByFence === 1 ? "" : "s"} sent nothing — the safety fence stopped them)`
        : "") +
      (s.messages.blockedByGate ? `  (${s.messages.blockedByGate} held by the compliance gate)` : ""),

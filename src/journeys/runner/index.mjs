...
import { dispatchQueued } from "../../messaging/dispatch.mjs";
...
async function walkPath(db, ctx, { journeyKey, journey, path, pathId, index }) {
  ...
  const { results: dispatched, truncated } = await dispatchQueued(db, {
    orgId,
    now: () => clock.now(),
    env
  });
  ...
  return {
    ...
    dispatched: dispatched.map((d) => ({ status: d.status, channel: d.channel, reason: d.reason ?? null, provider: d.provider ?? null })),

-- 091_bank_account_provider.sql — WHERE A BANK ACCOUNT ROW CAME FROM.
--
-- ═══════════════════════════════════════════════════════════════════════════
...
-- A row that names a bank connection must not claim a person typed it, and a
-- mock must never hang off a real Item. Both directions, because the failure
-- that matters is a mock row acquiring the look of a real one.
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE bank_accounts
      ...
      CHECK (
        (provider = 'plaid'  AND plaid_item_id IS NOT NULL)
        OR (provider <> 'plaid' AND plaid_item_id IS NULL)
      );
  END IF;
END $$;

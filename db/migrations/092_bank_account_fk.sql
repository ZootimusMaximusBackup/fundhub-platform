...
BEGIN
  IF NOT EXISTS (
    ...
    -- SET NULL, NOT CASCADE, AND THE COLUMN IS MADE NULLABLE TO ALLOW IT.
    --
    -- This file was written before migrations 101 and 102 existed. Its original
    -- CASCADE was chosen on the reasoning that SET NULL "is unavailable on a NOT
    -- NULL column" — true of the column as 085 declared it, but that is a
    -- property of the declaration, not a reason, and it is dropped here.
    --
    -- CASCADE cannot stand alongside what main now ships. 101 adds
    -- subject_client_id precisely so a transaction stays findable after the
    -- person's row is gone, and its own comment rules out this exact FK:
    -- "SET NULL reproduces the orphan bug, CASCADE destroys the ledger". 102's
    -- erasure path then de-identifies the ledger rather than deleting it. With
    -- CASCADE here, deleting a client removes the bank_accounts row (081:42) and
    -- takes every transaction with it, so 101's anchor anchors nothing and an
    -- erasure request reports a person erased whose history was destroyed
    -- instead of retained.
    --
    -- SET NULL keeps this file's actual goal — no row may point at a
    -- bank_accounts row that does not exist — while leaving the financial record
    -- in place. Revoking a bank login stops the sync and drops the account; it
    -- is not a request to erase history, and those two are separate operations
    -- in this schema on purpose.
    --
    -- recurring_bills below KEEPS CASCADE. A detected bill is derived from
    -- transactions and re-derivable from them; it is not the financial record.
    ...
    -- IS NOT NULL matters now the column is nullable: a NULL link is a row whose
    -- account was removed and whose record was deliberately kept, not an orphan.
    -- A foreign key never rejects NULL, so counting those would skip the VALIDATE
    -- below and raise a warning about rows that are in their intended state.
    SELECT count(*) INTO orphans
      FROM bank_transactions t
      LEFT JOIN bank_accounts a ON a.id = t.bank_account_id
     WHERE t.bank_account_id IS NOT NULL AND a.id IS NULL;
...
COMMENT ON COLUMN bank_transactions.bank_account_id IS
  'The bank_accounts row this transaction moved through. FK with ON DELETE SET NULL (092): removing the account — including when a revoked bank login cascades it away — empties this column and LEAVES THE TRANSACTION IN PLACE. NULL therefore means the account is gone, not that the row is malformed; the route back to the person is subject_client_id (101), which is deliberately not a foreign key. Do not restore CASCADE here: it destroys the financial record that 101 and 102 exist to keep findable and de-identifiable.';
